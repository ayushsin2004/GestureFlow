...
RECORDING_FILENAME = 'video/' + str(datetime.now()).replace('.','').replace(':','') + '.avi'
FPS = 10
    ...
    if pose_result.pose_landmarks:
      ...
      if is_squatting(hipL, kneeL, hipR, kneeR):
        ...
        if is_limb_pointing(*legL) and is_limb_pointing(*legR):
          ...
          if is_high is not None:# Ayush Singh - https://github.com/ayushsin2004
...
def main():
  ...
  parser.add_argument('--input', '-i', help='Input video device or file (number or path), defaults to 0', default='1')
